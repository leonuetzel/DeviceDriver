...
feedback USART_2::init(uint32 baud, e_databits databits, e_stopbits stopbits, e_parity parity, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	{
		DMA_1& dma_tx = stm32.get_dma_1(6);
		m_dma_tx = &dma_tx;
		dma_tx.set_requestMapping(DMA_1::e_requestMapping::CH6_USART2TX);
	}
	...
	{
		DMA_1& dma_rx = stm32.get_dma_1(5);
		m_dma_rx = &dma_rx;
		dma_rx.set_requestMapping(DMA_1::e_requestMapping::CH5_USART2RX);
	}
	...
	const uint32 clock_in = rcc.get_clock_usart_2();
...
void ISR_USART_2()
{
	USART_2& usart = STM32L451RET6::get().get_usart_2();
