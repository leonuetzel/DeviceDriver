...
feedback I2C_1::init(e_mode mode, RCC::e_clockSource_i2c clockSource, bool analogFilterEnable, uint8 digitalFilterLength)
{
	...
	RCC& rcc = STM32C071KBU6::get().get_rcc();
								...
								for(uint32 SCLL = 0; SCLL < 256; SCLL++)
								{
									if(t_low_min <= t_af_min + (DNF + 3) * t_i2cclk + (SCLL + 1) * t_PRESC)
									{
										if(SCLL == SCLH)
										{
											...
											*MCU::I2C_1::TIMINGR = timingRegister;
											
											
											//	Analog Filters
											if(analogFilterEnable == false)
											{
												bit::set(*MCU::I2C_1::CR1, 12);
											}
											else
											{
												bit::clear(*MCU::I2C_1::CR1, 12);
											}
											
											
											//	Digital Filters
											const uint32 temp = *MCU::I2C_1::CR1 & 0xFFFFF0FF;
											*MCU::I2C_1::CR1 = temp | (digitalFilterLength << 8);
	...
	*MCU::I2C_1::CR2 = 0x02003000 | (numberOfBytes << 16) | (writeMask << 10) | (slaveAddress << 1);
	...
	if(write == true)
	{
		...
		while(bit::isCleared(*MCU::I2C_1::ISR, 1))
		{
			...
			{
				//	Workaround for something, that worked on STM32C031K6U6
				...
				{
					if(bit::isCleared(*MCU::I2C_1::ISR, 15))
					{
						break;
					}
				}
