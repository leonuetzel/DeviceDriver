...
feedback USART_2::init(uint32 baud, e_databits databits, e_stopbits stopbits, e_parity parity, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	*MCU::USART_2::CR1 = temp | (((uint32) parity) << 9);
	...
	//	Rx DMA Setup
	if(m_dma_rx->transfer((void*) MCU::USART_2::DR, m_data, I_DMA::e_dataType::UINT8, I_DMA::e_dataType::UINT8, false, true, I_DMA::e_direction::PERIPHERAL_TO_MEMORY, true, m_size, I_DMA::e_priority::VERY_HIGH, false) != OK)
	{
		return(FAIL);
	}
...
void ISR_USART_2()
{
	USART_2& usart = STM32F107RCT6::get().get_usart_2();
	
	
	//  IDLE Line detected
	if(bit::isSet(*MCU::USART_2::SR, 4))
	{
		...
		*MCU::USART_2::DR;
		...
		//	Update Head
		usart.m_head = usart.m_size - *MCU::DMA_1::CH5_NDTR;
