...
feedback USART_2::init(uint32 baud, e_databits databits, e_parity parity, e_stopbits stopbits, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	STM32G474MCT6& stm32 = STM32G474MCT6::get();
	{
		...
		dmamux.map(DMAMUX::e_input::USART_2_TX, DMAMUX::e_output::DMA_1_CH6);
	}
	...
	//	Lock Semaphore of Rx DMA
	{
		...
		m_dma_rx = &dma_rx;
		DMAMUX& dmamux = stm32.get_dmamux();
		dmamux.map(DMAMUX::e_input::USART_2_RX, DMAMUX::e_output::DMA_1_CH5);
	}
...
void ISR_USART_2()
{
	USART_2& usart = STM32G474MCT6::get().get_usart_2();
