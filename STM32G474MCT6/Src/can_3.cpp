...
feedback CAN_3::init(uint32 baudRate, const Array<s_filterElement>& standardfilterElements, const Array<s_filterElement>& extendedfilterElements)
{
	...
	bit::set(*MCU::FDCAN_3::CCCR, 0);
	bit::set(*MCU::FDCAN_3::CCCR, 1);
	...
	*MCU::FDCAN_3::RXGFC = (0x03 << 4) | (0x03 << 2);
	*MCU::FDCAN_3::RXGFC |= (standardfilterElements.get_size() << 16);
	*MCU::FDCAN_3::RXGFC |= (extendedfilterElements.get_size() << 24);
	...
	for(uint32 i = 0; i < standardfilterElements.get_size(); i++)
	{
		...
		*(MCU::FDCAN_3::MSG_RAM + i) = filterElementInRAM;
	}
	...
	for(uint32 i = 0; i < extendedfilterElements.get_size(); i++)
	{
		...
		//	Write Filter Element to Message RAM
		*(MCU::FDCAN_3::MSG_RAM + 28 + (i * 2)) = filterElementInRAM[0];
	}
	...
	*MCU::FDCAN_3::IE = 0x00EF407F;
	
	
	//	INT0: All RX FIFO related Interrupts
	//	INT1: All Error Interrupts
	*MCU::FDCAN_3::ILS = 0x00000078;
	
	
	//	Enable Interrupt INT0 and INT1
	*MCU::FDCAN_3::ILE = 0x00000003;
	
	NVIC& nvic = cmos.get_nvic();
	nvic.setPriority(Interrupt::FDCAN_3_IT0, 10);
...
feedback CAN_3::tx(const CAN_Frame& canFrame)
{
	//	Check if Tx Buffer is full
	if(bit::isSet(*MCU::FDCAN_3::TX_FQS, 21) == true)
	{
		//	Tx Buffer is full
	...
	const uint32 putIndex = ((*MCU::FDCAN_3::TX_FQS & 0x00030000) >> 16);
	volatile uint32* T = MCU::FDCAN_3::MSG_RAM + 0x9E + 0x12 * putIndex;
	...
	bit::set(*MCU::FDCAN_3::TX_BAR, putIndex);
...
feedback CAN_3::rx(CAN_Frame& canFrame, uint32 fifoID)
{
	if(fifoID > 1)
	{
		return(FAIL);
	...
	while(1)
	{
		...
		volatile uint32* RX_FnS = MCU::FDCAN_3::RX_F0S;
		if(fifoID == 1)
		{
			RX_FnS = MCU::FDCAN_3::RX_F1S;
		}
		const uint32 getIndex = (*RX_FnS & 0x00000300) >> 8;
		volatile uint32* const R = MCU::FDCAN_3::MSG_RAM + 0x2C + 0x36 * fifoID + 0x12 * getIndex;
		...
		volatile uint32* RX_FnA = MCU::FDCAN_3::RX_F0A;
		if(fifoID == 1)
		{
			RX_FnA = MCU::FDCAN_3::RX_F1A;
		}
		*RX_FnA = getIndex;
...
uint32 CAN_3::get_numberOfUnread(uint32 fifoID) const
{
	if(fifoID == 0)
	{
		return(*MCU::FDCAN_3::RX_F0S & 0x0000000F);
	}
	if(fifoID == 1)
	{
		return(*MCU::FDCAN_3::RX_F1S & 0x0000000F);
	}
	return(0);
}


bool CAN_3::is_dataAvailable() const
{
...
uint16 CAN_3::get_eventID()
{
	return(m_eventID_frameReadyToRead);
}


CAN_3::e_state CAN_3::get_state()
{
	//	CAN State Machine
	...
	const uint32 PSR = *MCU::FDCAN_3::PSR;
...
const UniquePairArray<CAN_3::e_error, bool>& CAN_3::get_errors() const
{
	return(m_errors);
}


void CAN_3::clearErrors()
{
	for(auto& i: m_errors)
	{
		i.second() = false;
...
uint32 CAN_3::get_baudRate() const
{
	return(m_baudRate);
}
...
void ISR_FDCAN_3_IT0()
{
	if(CAN_3::m_this != nullptr)
	{
		CAN_3& can = *CAN_3::m_this;
		
		
		//	Read Interrupt Status Register
		const uint32 IR = *MCU::FDCAN_3::IR;
		
		
		//	High Priority Message received
		if(bit::isSet(IR, 6) == true)
		{
			//	Emit Event so that the CAN_3::rx() Function can read the Frame
