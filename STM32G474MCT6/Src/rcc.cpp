...
const Pair<PWR::e_voltageScaling, RCC::s_clockBoundaries> c_clockBoundaries[3] =
{
	Pair<PWR::e_voltageScaling, RCC::s_clockBoundaries>
	(
		PWR::e_voltageScaling::RANGE1_1V28,
		RCC::s_clockBoundaries
		{
			...
			.c_clock_pll_p_min =					  2064500,
			.c_clock_pll_p_max =					170000000,
			.c_clock_pll_q_min =					  8000000,
			.c_clock_pll_q_max =					170000000,
			.c_clock_pll_r_min =					  8000000,
			.c_clock_pll_r_max =					170000000
		}
	),
	Pair<PWR::e_voltageScaling, RCC::s_clockBoundaries>
	(
		...
		RCC::s_clockBoundaries
		{
			...
			.c_clock_pll_vco_max =				344000000,
			.c_clock_pll_p_min =					  2064500,
			.c_clock_pll_p_max =					150000000,
			.c_clock_pll_q_min =					  8000000,
			.c_clock_pll_q_max =					150000000,
			.c_clock_pll_r_min =					  8000000,
			.c_clock_pll_r_max =					150000000
		}
	),
	Pair<PWR::e_voltageScaling, RCC::s_clockBoundaries>
	(
...
{
	//	RM0440 - Rev. 9, Chapter 7.2, Figure 17, Page 184
	...
	{
		case e_clockSource_pll::NONE:
		{
			m_clock_pll_input = 0;
		}
		break;
		
		case e_clockSource_pll::HSI16:
		{
			m_clock_pll_input = m_clock_hsi16 / m_prescaler_pll_input;
		}
		break;
		
		case e_clockSource_pll::HSE:
		{
			m_clock_pll_input = m_clock_hse / m_prescaler_pll_input;
		}
		break;
	...
	{
		m_clock_pll = m_clock_pll_input * m_multiplier_pll;
	}
	else
	{
		m_clock_pll = 0;
...
feedback RCC::set_clockSource(e_clockSource_pll clockSource)
{
	...
	const e_clockSource_pll m_clockSource_pll_old = m_clockSource_pll;
	...
	if(m_clock_pll_input > c_clock_pll_input_max || m_clock_pll_input < c_clock_pll_input_min)
	{
		//	Undo Changes
		m_pll_enabled = pll_enabled_old;
		m_clockSource_pll = m_clockSource_pll_old;
		calculate_clockTree();
...
feedback RCC::init_pll(uint32 clock_out, uint32 prescaler_p, uint32 prescaler_q, uint32 prescaler_r)
{
	...
	const uint32 multiplier = clock_out / m_clock_pll_input;
