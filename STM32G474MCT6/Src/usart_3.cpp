...
feedback USART_3::init(uint32 baud, e_databits databits, e_parity parity, e_stopbits stopbits, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	{
		DMA_1& dma_tx = stm32.get_dma_1(1);
		m_dma_tx = &dma_tx;
		DMAMUX& dmamux = stm32.get_dmamux();
		dmamux.map(DMAMUX::e_input::USART_3_TX, DMAMUX::e_output::DMA_1_CH1);
	}
	...
	{
		DMA_1& dma_rx = stm32.get_dma_1(2);
		m_dma_rx = &dma_rx;
		DMAMUX& dmamux = stm32.get_dmamux();
		dmamux.map(DMAMUX::e_input::USART_3_RX, DMAMUX::e_output::DMA_1_CH2);
	}
	...
	const uint32 clock_in = rcc.get_clock_usart_3();
	...
	*MCU::USART_3::CR1 = temp | (((uint32) parity) << 9);
	...
	//	Rx DMA Setup
	if(m_dma_rx->transfer((void*) MCU::USART_3::RDR, m_data, I_DMA::e_dataType::UINT8, I_DMA::e_dataType::UINT8, false, true, I_DMA::e_direction::PERIPHERAL_TO_MEMORY, true, m_size, I_DMA::e_priority::VERY_HIGH, false) != OK)
	{
		return(FAIL);
	}
	...
	nvic.setPriority(Interrupt::USART_3, 2);
	nvic.enable(Interrupt::USART_3);
	
	
	//	USART Enable
	bit::set(*MCU::USART_3::CR1, 0);
...
void ISR_USART_3()
{
	USART_3& usart = STM32G474MCT6::get().get_usart_3();
	
	
	//  IDLE Line detected
	if(bit::isSet(*MCU::USART_3::ISR, 4))
	{
	...
	//	Rx Buffer Overrun
	if(bit::isSet(*MCU::USART_3::ISR, 3))
	{
		//	Clear Interrupt Flag
		*MCU::USART_3::ICR = 1 << 3;
