...
feedback USART_1::init(uint32 baud, e_databits databits, e_parity parity, e_stopbits stopbits, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	STM32G474MCT6& stm32 = STM32G474MCT6::get();
	{
		...
		dmamux.map(DMAMUX::e_input::USART_1_TX, DMAMUX::e_output::DMA_1_CH3);
	}
	...
	//	Lock Semaphore of Rx DMA
	{
		...
		m_dma_rx = &dma_rx;
		DMAMUX& dmamux = stm32.get_dmamux();
		dmamux.map(DMAMUX::e_input::USART_1_RX, DMAMUX::e_output::DMA_1_CH4);
	}
...
void ISR_USART_1()
{
	USART_1& usart = STM32G474MCT6::get().get_usart_1();
