...
inline feedback PWR::set_voltageScaling(e_voltageScaling voltageScaling)
{
	...
	const uint32 mask = ((uint32) voltageScaling) & 0x00000003;
	*MCU::PWR::CR1 = temp | (mask << 9);
	
	if(bit::isCleared((uint32) voltageScaling, 4) == true)
	{
		bit::set(*MCU::PWR::CR5, 8);
	}
	else
	{
		bit::clear(*MCU::PWR::CR5, 8);
	}
...
inline feedback PWR::voltageScalingIncrease()
{
	...
	{
		return(set_voltageScaling(e_voltageScaling::RANGE1_1V28));
	}
	return(FAIL);
}
...
{
	if(m_voltageScaling == e_voltageScaling::RANGE1_1V28)
	{
		return(set_voltageScaling(e_voltageScaling::RANGE1_1V2));
	}
	if(m_voltageScaling == e_voltageScaling::RANGE1_1V2)
