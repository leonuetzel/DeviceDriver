...
feedback CAN_1::init(uint32 baudRate, const Array<s_filterElement>& standardfilterElements, const Array<s_filterElement>& extendedfilterElements, bool silentMode, uint32 rxBufferSize, uint32 txBufferSize)
{
	...
	bit::set(*MCU::FDCAN_1::FDCAN::CCCR, 1);
	...
	*MCU::FDCAN_1::FDCAN::NBTP = (SJW << 25) | (bestNBRP << 16) | (bestNTSEG1 << 8) | bestNTSEG2;
	
	
	//	Silent Mode
	if(silentMode == true)
	{
		bit::set(*MCU::FDCAN_1::FDCAN::CCCR, 5);
	}
	else
	{
		bit::clear(*MCU::FDCAN_1::FDCAN::CCCR, 5);
	}
	...
	constexpr uint32 offsetCAN1MessageRAMInWords					= 0;
	...
	constexpr uint32 offsetStandardFilterElementsInWords	= offsetCAN1MessageRAMInWords;
	...
	*MCU::FDCAN_1::FDCAN::SIDFC = (numberOfStandardFilterElements << 16) | (offsetStandardFilterElementsInWords << 2);
	...
	*MCU::FDCAN_1::FDCAN::XIDFC = (numberOfExtendedFilterElements << 16) | (offsetExtendedFilterElementsInWords << 2);
	...
	*MCU::FDCAN_1::FDCAN::RXF0C = (0x00 << 31) | (0x00 << 24) | (numberOfRxFIFOElements << 16) | (offsetRxFIFOElementsInWords << 2);
	...
	*MCU::FDCAN_1::FDCAN::TXBC = (0x01 << 30) | (numberOfTxFIFOElements << 24) | (0x00 << 16) | (offsetTxFIFOElementsInWords << 2);
	...
	*MCU::FDCAN_1::FDCAN::IE = 0x3BC2000D;
	
	
	//	Enable Interrupt INT0
	*MCU::FDCAN_1::FDCAN::ILE = 0x00000001;
	
	NVIC& nvic = cmos.get_nvic();
	nvic.setPriority(Interrupt::FDCAN_1_INT0, 10);
	nvic.enable(Interrupt::FDCAN_1_INT0);
...
feedback CAN_1::tx(const CAN_Frame& canFrame)
{
	...
	if(bit::isSet(*MCU::FDCAN_1::FDCAN::TXFQS, 21) == true)
	{
		//	Tx Buffer is full
	...
	volatile uint32* T = MCU::FDCAN_MSG_RAM + ((*MCU::FDCAN_1::FDCAN::TXBC & 0x0000FFFC) >> 2);
	...
	const uint32 putIndex = (*MCU::FDCAN_1::FDCAN::TXFQS & 0x001F0000) >> 16;
	...
	bit::set(*MCU::FDCAN_1::FDCAN::TXBAR, putIndex);
...
CAN_1::e_state CAN_1::get_state()
{
	...
	const uint32 PSR = *MCU::FDCAN_1::FDCAN::PSR;
...
void ISR_FDCAN_1_INT0()
{
	if(CAN_1::m_this != nullptr)
	{
		...
		const uint32 IR = *MCU::FDCAN_1::FDCAN::IR;
		...
		if(bit::isSet(IR, 29) == true)
		{
			...
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 29);
		}
		if(bit::isSet(IR, 28) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 28);
		}
		if(bit::isSet(IR, 27) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 27);
		}
		if(bit::isSet(IR, 25) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 25);
		}
		if(bit::isSet(IR, 24) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 24);
		}
		if(bit::isSet(IR, 23) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 23);
		}
		if(bit::isSet(IR, 22) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 22);
		}
		if(bit::isSet(IR, 17) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 17);
		}
		if(bit::isSet(IR, 3) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 3);
		}
		if(bit::isSet(IR, 2) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 2);
		}
		if(bit::isSet(IR, 0) == true)
		{
			...
			//	Clear the Interrupt Flag
			bit::set(*MCU::FDCAN_1::FDCAN::IR, 0);
		}
	}
}