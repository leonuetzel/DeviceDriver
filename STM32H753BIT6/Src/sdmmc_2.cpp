...
feedback SDMMC_2::set_clockSpeed(RCC& rcc, uint32 clock)
{
	uint32 clock_in = 0;
	...
	uint32 temp = *MCU::SDMMC_2::CLKCR & 0xFFFFFC00;
	*MCU::SDMMC_2::CLKCR = temp | (prescaler / 2);
	
	return(OK);
}


feedback SDMMC_2::set_busWidth(e_busWidth busWidth)
{
	...
	uint32 temp = *MCU::SDMMC_2::CLKCR & 0xFFFF3FFF;
	*MCU::SDMMC_2::CLKCR = temp | (((uint32) busWidth) << 14);
...
CODE_RAM SDMMC_2::s_response SDMMC_2::send_command(s_commandPath command) const
{
	CMOS& cmos = CMOS::get();
	...
	while(bit::isSet(*MCU::SDMMC_2::STAR, 13))																																										//	Wait for Control Path to be in IDLE State
	{
		cmos.sleep_ms(1);
	}
	
	*MCU::SDMMC_2::ICR = *MCU::SDMMC_2::STAR;																																											//	Clear Flags
	*MCU::SDMMC_2::ARGR = command.argument;																																												//	Set Argument
	
	
	
	if(command.dataTransfer == true)																																															//	Send Command
	{
		*MCU::SDMMC_2::CMDR = 0x00001040 | ((uint32) command.command);
	}
	else
	{
		*MCU::SDMMC_2::CMDR = 0x00001000 | ((uint32) command.command);
	}
	
	while(bit::isSet(*MCU::SDMMC_2::STAR, 13))																																										//	Wait for Command to be transmitted
	{
		cmos.sleep_ms(1);
	}
	while(bit::isSet(*MCU::SDMMC_2::STAR, 12))																																										//	Wait for Data to be transmitted
	{
		cmos.sleep_ms(1);
	}
	
	
	uint32 status = *MCU::SDMMC_2::STAR;
	switch((((uint32) command.command) & 0x00000300) >> 8)
	{
		...
		case 0x1:
			...
			{
				response.response_31_0 = *MCU::SDMMC_2::RESP_1R;
				response.success = OK;
			}
			break;
		...
		case 0x3:
			if(bit::isSet(status, 6))
			{
				response.response_127_96 = *MCU::SDMMC_2::RESP_1R;
				response.response_95_64 = *MCU::SDMMC_2::RESP_2R;
				response.response_63_32 = *MCU::SDMMC_2::RESP_3R;
				response.response_31_0 = *MCU::SDMMC_2::RESP_4R;
				response.success = OK;
			}
			break;
	...
	*MCU::SDMMC_2::ICR = status;
	return(response);
}


CODE_RAM void SDMMC_2::setup_dataPath(s_dataPath dataPath) const
{
	*MCU::SDMMC_2::DLENR = dataPath.data_sizeInBytes;
	*MCU::SDMMC_2::DTIMER = dataPath.timeout;
	*MCU::SDMMC_2::DCTRLR = ((uint32) dataPath.blockSize << 4) | ((uint32) dataPath.mode << 2) | ((uint32) dataPath.direction << 1);
}
...
feedback SDMMC_2::command_reset()
{
...
feedback SDMMC_2::command_operatingConditionValidation()
{
	s_commandPath command;
	s_response response;
...
feedback SDMMC_2::command_set_operatingCondition()
{
	s_commandPath command;
	s_response response;
...
feedback SDMMC_2::command_request_CID()
{
	s_commandPath command;
	s_response response;
...
feedback SDMMC_2::command_publish_newRCA()
{
	s_commandPath command;
	s_response response;
...
feedback SDMMC_2::command_request_CSD()
{
	s_commandPath command;
	s_response response;
...
feedback SDMMC_2::command_cardSelect()
{
	s_commandPath command;
	s_response response;
...
feedback SDMMC_2::command_set_busWidth()
{
	s_commandPath command;
	s_response response;
...
feedback SDMMC_2::command_send_SDStatus()
{
	s_commandPath command;
	s_response response;
	s_dataPath dataPath;
	...
	bit::clear(*MCU::SDMMC_2::IDMA_CTRLR, 0);																																											//	Disable IDMA
	*MCU::SDMMC_2::IDMA_BASE0R = (uint32) data;																																										//	Set IDMA Buffer Address
	bit::set(*MCU::SDMMC_2::IDMA_CTRLR, 0);																																												//	Enable IDMA
...
CODE_RAM feedback SDMMC_2::command_read_singleBlock(uint32 block) const
{
	s_commandPath command;
	s_response response;
	s_dataPath dataPath;
	
	*MCU::SDMMC_2::IDMA_BSIZER = 512;
...
CODE_RAM feedback SDMMC_2::command_write_singleBlock(uint32 block) const
{
	s_commandPath command;
	s_response response;
	s_dataPath dataPath;
...
feedback SDMMC_2::init(RCC& rcc, RCC::e_clockSource_sdmmc clockSource, f_power power, f_detect detect)
{
	CMOS& cmos = CMOS::get();
	...
	rcc.module_reset(RCC::e_module::SDMMC_2);
	if(rcc.set_clockSource(clockSource) != OK)
	{
		return(FAIL);
	}
	m_clockSource = clockSource;
	rcc.module_clockInit(RCC::e_module::SDMMC_2, true);
	...
	*MCU::SDMMC_2::POWER = 0x00000003;																																														//	Put Power to the Card
	bit::set(*MCU::SDMMC_2::CLKCR, 17);																																														//	Enable Hardware Flow Control
...
CODE_RAM feedback SDMMC_2::enable()
{
	if(m_power(true) != OK)
	{
		m_enabled = false;
...
CODE_RAM feedback SDMMC_2::disable()
{
	if(m_power(false) != OK)
	{
		m_enabled = true;
...
CODE_RAM bool SDMMC_2::detect() const
{
	if(m_detect != nullptr)
	{
		return(m_detect());
...
CODE_RAM uint32 SDMMC_2::get_changeStamp() const
{
	return(m_changeCounter);
}


CODE_RAM uint8* SDMMC_2::read(uint32 startSector, uint32 numberOfSectors) const
{
	...
	*/
	...
	{
		bit::clear(*MCU::SDMMC_2::IDMA_CTRLR, 0);																																										//	Disable IDMA
		*MCU::SDMMC_2::IDMA_BASE0R = ((uint32) data) + i * c_sectorSizeInBytes;																											//	Set IDMA Buffer Address
		bit::set(*MCU::SDMMC_2::IDMA_CTRLR, 0);																																											//	Enable IDMA
...
CODE_RAM feedback SDMMC_2::read(uint32 startSector, uint32 numberOfSectors, uint8* target) const
{/*
	Cache& cache = CMOS::get().get_cache();																																											//	Cache Maintenance
	Thread& self = CMOS::get().get_thread();
	self.set_priviledgeLevel(Thread::e_priviledgeLevel::ADMIN);
	...
	*/
	...
	{
		bit::clear(*MCU::SDMMC_2::IDMA_CTRLR, 0);																																										//	Disable IDMA
		*MCU::SDMMC_2::IDMA_BASE0R = ((uint32) target) + i * c_sectorSizeInBytes;																										//	Set IDMA Buffer Address
		bit::set(*MCU::SDMMC_2::IDMA_CTRLR, 0);																																											//	Enable IDMA
...
CODE_RAM feedback SDMMC_2::write(uint32 startSector, uint32 numberOfSectors, uint8* data) const
{/*
	Cache& cache = CMOS::get().get_cache();																																											//	Cache Maintenance
	Thread& self = CMOS::get().get_thread();
	self.set_priviledgeLevel(Thread::e_priviledgeLevel::ADMIN);
