...
feedback USART_1::init(uint32 baud, e_databits databits, e_stopbits stopbits, e_parity parity, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	rcc.set_clockSource(RCC::e_clockSource_uart::BUS_CLOCK, 1);
	...
	*MCU::USART_1::CR1 = temp | (((uint32) parity) << 9);
	...
	dmamux.map(DMAMUX::e_input::USART_1_RX_DMA, DMAMUX::e_output::DMA_1_CH3);
	dmamux.map(DMAMUX::e_input::USART_1_TX_DMA, DMAMUX::e_output::DMA_1_CH4);
...
CODE_RAM void ISR_USART_1()
{
	USART_1& usart = STM32H753BIT6::get().get_usart_1();
	...
	if(bit::isSet(*MCU::USART_1::ISR, 4))
	{
		...
		bit::set(*MCU::USART_1::ICR, 4);
		
		
		//	Update Head
		usart.m_head = usart.m_size - *MCU::DMA_1::CH3_NDTR;
