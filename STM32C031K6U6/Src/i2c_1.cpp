...
feedback I2C_1::init(e_mode mode, RCC::e_clockSource_i2c clockSource, bool analogFilterEnable, uint8 digitalFilterLength)
{
	//	Boundary Check
	...
	RCC& rcc = STM32C031K6U6::get().get_rcc();
	rcc.set_clockSource(clockSource);
	uint32 clock_in = 0;
	switch(clockSource)
	{
		case RCC::e_clockSource_i2c::SYSTEM:
		{
			clock_in = rcc.get_clock_system();
		}
		break;
		
		case RCC::e_clockSource_i2c::HSI_KERNEL:
		{
			clock_in = rcc.get_clock_hsiKernel();
		}
		break;
		
		case RCC::e_clockSource_i2c::APB:
		{
			clock_in = rcc.get_clock_apb();
		}
		break;
								...
								for(uint32 SCLL = 0; SCLL < 256; SCLL++)
								{
									if(t_low_min <= t_af_min + (DNF + 3) * t_i2cclk + (SCLL + 1) * t_PRESC)
									{
										if(SCLL == SCLH)
										{
											...
											*MCU::I2C::TIMINGR = timingRegister;
											
											
											//	Analog Filters
											if(analogFilterEnable == false)
											{
												bit::set(*MCU::I2C::CR1, 12);
											}
											else
											{
												bit::clear(*MCU::I2C::CR1, 12);
											}
											
											
											//	Digital Filters
											const uint32 temp = *MCU::I2C::CR1 & 0xFFFFF0FF;
											*MCU::I2C::CR1 = temp | (digitalFilterLength << 8);
											
											
											//	Enable I2C
											bit::set(*MCU::I2C::CR1, 0);
...
CODE_RAM feedback I2C_1::start(uint8 slaveAddress, bool write, uint8 numberOfBytes, uint32 timeout_ms)
{
	...
	*MCU::I2C::CR2 = 0x02003000 | (numberOfBytes << 16) | (writeMask << 10) | (slaveAddress << 1);
	...
	if(write == true)
	{
		...
		while(bit::isCleared(*MCU::I2C::ISR, 1))
		{
			//	Check for Timeout
			...
			if(bit::isSet(*MCU::I2C::ISR, 4))
			{
				//	Clear NACK Flag
				bit::set(*MCU::I2C::ICR, 4);
				return(FAIL);
			}
		}
	}
	else
	{
		...
		while(bit::isCleared(*MCU::I2C::ISR, 2))
		{
			//	Check for Timeout
			const uint32 timeWaited_ms = (ticks - timestampStart) / (CMOS::c_clock_systick / 1000);
			if(timeWaited_ms > timeout_ms)
...
CODE_RAM uint8 I2C_1::rx(uint32 timeout_ms)
{
	...
	uint8 data = *MCU::I2C::RXDR & 0x000000FF;
