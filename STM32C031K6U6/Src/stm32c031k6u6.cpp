...
INTERRUPT_VECTOR const STM32C031K6U6::s_interruptTable STM32C031K6U6::c_interruptTable =
{
	...
	(uint32) 0,                                                                                																		// 0x0A0 Reserved
	(uint32) ISR_SPI_1,                                                                        																		// 0x0A4 SPI 1
	(uint32) 0,                                                                                																		// 0x0A8 Reserved
...
void STM32C031K6U6::copy_flashToRAM(const uint32* src, uint32* dest, uint32 sizeInBytes)
{
...
void STM32C031K6U6::zeroOutRAM(uint32* startAddress, uint32 sizeInBytes)
{
	for(uint32 i = 0; i < sizeInBytes / 4; i++)
	{
		*(startAddress + i) = 0;
...
void EXCEPTION_RESET()
{
	...
	STM32C031K6U6::zeroOutRAM(c_memory_SRAM_start, c_memory_SRAM_size - 128);
	
	
	//	Copy non-zero-initialized Variables to SRAM
	STM32C031K6U6::copy_flashToRAM(c_data_flash_start, c_data_ram_start, c_data_flash_size);
	
	
	//	Copy Code from Flash to SRAM
	STM32C031K6U6::copy_flashToRAM(c_code_flash_start, c_code_ram_start, c_code_flash_size);
	
	
	//	Execute Functions that need to be executed before any Initialization has been done (Bootloader)
	STM32C031K6U6::preInit();
...
WEAK ALIAS(__dummyInterruptHandler__) NOTHROW void ISR_DMAMUX();
...
uint32 __startup__()
{
	STM32C031K6U6& stm32f107rct6 = STM32C031K6U6::get();
