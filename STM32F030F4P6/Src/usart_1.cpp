...
feedback USART_1::init(uint32 baud, e_databits databits, e_stopbits stopbits, e_parity parity, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	STM32F030F4P6& stm32 = STM32F030F4P6::get();
	...
	rcc.set_clockSource(RCC::e_clockSource_usart_1::SYSTEM_CLOCK);
	...
	//	Baud Rate
	*MCU::USART_1::BRR = rcc.get_clock_system() / baud;
	...
	*MCU::USART_1::CR1 = temp | (((uint32) parity) << 9);
	...
	//	Subscribe to USART Rx Message Receive Event so that we get woken up by the ISR when Data is received
...
void ISR_USART_1()
{
	USART_1& usart = STM32F030F4P6::get().get_usart_1();
