...
INTERRUPT_VECTOR const STM32F030F4P6::s_interruptTable STM32F030F4P6::c_interruptTable =
{
	...
	(uint32) ISR_DMA_CH3_CH4,                                                                    																	// 0x6C DMA Channel 3 - 4
	(uint32) ISR_ADC,                                                                            																	// 0x70 ADC
	(uint32) ISR_TIMER_1_BREAK_UPDATE_TRIGGER_COMMUTATION,                                       																	// 0x74 Timer 1 BRK, UPD, TRG and COMM
	(uint32) ISR_TIMER_1_CAPTURE_COMPARE,                                                        																	// 0x78 Timer 1 Capture Compare
	(uint32) 0,                                                                                  																	// 0x7C Reserved
	(uint32) ISR_TIMER_3,                                                                        																	// 0x80 Timer 3
	(uint32) 0,                                                                                  																	// 0x84 Reserved
	(uint32) 0,                                                                                  																	// 0x88 Reserved
	(uint32) ISR_TIMER_14,                                                                       																	// 0x8C Timer 14
	(uint32) 0,                                                                                  																	// 0x90 Reserved
	(uint32) ISR_TIMER_16,                                                                       																	// 0x94 Timer 16
	(uint32) ISR_TIMER_17,                                                                       																	// 0x98 Timer 17
	(uint32) ISR_I2C_1,                                                                          																	// 0x9C I2C 1
	(uint32) 0,                                                                                  																	// 0xA0 Reserved
	(uint32) ISR_SPI_1,                                                                          																	// 0xA4 SPI 1
	(uint32) 0,                                                                                 																	// 0xA8 Reserved
	(uint32) ISR_USART_1,                                                                        																	// 0xAC USART 1
	(uint32) 0,                                                                                  																	// 0xB0 Reserved
...
void STM32F030F4P6::copy_flashToRAM(const uint32* src, uint32* dest, uint32 sizeInBytes)
{
...
void STM32F030F4P6::zeroOutRAM(uint32* startAddress, uint32 sizeInBytes)
{
	for(uint32 i = 0; i < sizeInBytes / 4; i++)
	{
		*(startAddress + i) = 0;
...
STM32F030F4P6& STM32F030F4P6::get()
{
	static STM32F030F4P6 stm32f030f4p6;
	return(stm32f030f4p6);
}
...
void EXCEPTION_RESET()
{
	...
	STM32F030F4P6::zeroOutRAM(c_memory_SRAM_start, c_memory_SRAM_size - c_stackpointerMemoryInBytes);
	...
	STM32F030F4P6::zeroOutRAM(c_memory_SRAM_start + (c_memory_SRAM_size - c_stackpointerMemoryInBytes) / 4, c_stackpointerMemoryInBytes);
	
	
	//	Copy non-zero-initialized Variables to SRAM
	STM32F030F4P6::copy_flashToRAM(c_data_flash_start, c_data_ram_start, c_data_flash_size);
	
	
	//	Copy Code from Flash to SRAM
	STM32F030F4P6::copy_flashToRAM(c_code_flash_start, c_code_ram_start, c_code_flash_size);
...
WEAK ALIAS(__dummyInterruptHandler__) NOTHROW void ISR_DMA_CH3_CH4();
...
uint32 __startup__()
{
	...
	if(stm32f030f4p6.startup() != OK)
	{
		return(0);
	}
	return(stm32f030f4p6.get_rcc().get_clock_system());
}