...
feedback USART_1::init(uint32 baud, e_databits databits, e_stopbits stopbits, e_parity parity, uint8* rxBuffer, uint32 rxBufferSize, uint8* txBuffer, uint32 txBufferSize)
{
	...
	STM32F103C8T6& stm32 = STM32F103C8T6::get();
	m_dma_tx = &stm32.get_dma(3);
	...
	const uint32 clock_in = rcc.get_clock_apb2();
	...
	*MCU::USART_1::CR1 = temp | (((uint32) parity) << 9);
	...
	//	Rx DMA Setup
	if(m_dma_rx->transfer((void*) MCU::USART_1::DR, m_data, I_DMA::e_dataType::UINT8, I_DMA::e_dataType::UINT8, false, true, I_DMA::e_direction::PERIPHERAL_TO_MEMORY, true, m_size, I_DMA::e_priority::VERY_HIGH, false) != OK)
	{
		return(FAIL);
	}
	...
	nvic.setPriority(Interrupt::USART_1, 10);
	nvic.enable(Interrupt::USART_1);
	
	
	//	USART Enable
	bit::set(*MCU::USART_1::CR1, 13);
...
void ISR_USART_1()
{
	USART_1& usart = STM32F103C8T6::get().get_usart_1();
	
	
	//  IDLE Line detected
	if(bit::isSet(*MCU::USART_1::SR, 4))
	{
